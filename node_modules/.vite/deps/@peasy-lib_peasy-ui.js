...
var UIView = class _UIView {
  ...
  get lastElement() {
    var _a, _b;
    if (this.render == null)
      this.render = !(((_b = (_a = this.element).hasAttribute) == null ? void 0 : _b.call(_a, "PUI-UNRENDERED")) ?? false);
    if (this.render)
      return this.element;
    const t2 = this.bindings.flatMap((t3) => t3.views);
    const e2 = t2.slice(-1)[0];
    return e2 == null ? void 0 : e2.lastElement;
  }
  destroy() {
    var _a;
    this.views.forEach((t2) => t2.destroy());
    (_a = this.element.classList) == null ? void 0 : _a.add("pui-removing");
  ...
  move(t2) {
    var _a;
    this.moved = "queue";
    (_a = this.element.classList) == null ? void 0 : _a.add("pui-moving");
    this.sibling = t2;
  }
  play(t2, e2) {
    if (typeof t2 === "string")
  ...
  updateToUI() {
    var _a, _b, _c, _d, _e;
    const t2 = this.state === "created";
    if (!t2) {
      this.views.forEach((t3) => t3.updateToUI());
      this.bindings.forEach((t3) => t3.updateToUI());
    }
    switch (this.state) {
      case "created":
        (_a = this.element.classList) == null ? void 0 : _a.add("pui-adding");
        this.render = !(((_c = (_b = this.element).hasAttribute) == null ? void 0 : _c.call(_b, "PUI-UNRENDERED")) ?? false);
        if (this.render) {
          const t3 = (_d = this.parent) == null ? void 0 : _d.parent;
          const e2 = (t3 == null ? void 0 : t3.render) ?? false;
          let i2 = (!e2 ? t3 == null ? void 0 : t3.sibling : this.sibling) ?? null;
          i2 = (i2 instanceof _UIView ? i2.lastElement : i2) ?? null;
          const n = this.parentElement ?? s.parentElement(this.element, this.parent);
          n.insertBefore(this.element, (i2 == null ? void 0 : i2.nextSibling) ?? null);
        }
        this.attachResolve();
      ...
      case "attaching":
        if (this.getAnimations(false).length === 0) {
          (_e = this.element.classList) == null ? void 0 : _e.remove("pui-adding");
          this.state = "attached";
        }
        break;
      case "attached":
    ...
    switch (this.moved) {
      ...
      case "move":
        if (this.getAnimations().length === 0) {
          const t2 = this.parentElement ?? s.parentElement(this.element, this.parent);
          let e2 = (this.sibling instanceof _UIView ? this.sibling.lastElement : this.sibling) ?? null;
          if (this.render)
            t2.insertBefore(this.element, (e2 == null ? void 0 : e2.nextSibling) ?? null);
          else {
            const i2 = this.bindings.flatMap((t3) => t3.views);
            for (const s2 of i2) {
              t2.insertBefore(s2.element, (e2 == null ? void 0 : e2.nextSibling) ?? null);
              e2 = s2.element;
            }
          }
          (_a = this.element.classList) == null ? void 0 : _a.remove("pui-moving");
          this.moved = "";
          this.sibling = null;
        }
        break;
  ...
  getAnimations(t2 = true) {
    var _a, _b;
    return (((_b = (_a = this.element).getAnimations) == null ? void 0 : _b.call(_a, {
      subtree: t2
    })) ?? []).filter((t3) => {
      var _a2;
      return t3.playState !== "finished" && ((_a2 = t3.effect) == null ? void 0 : _a2.getTiming().iterations) !== 1 / 0;
    }).map((t3) => t3.finished);
  }
};
var UIBinding = class _UIBinding {
  ...
  static create(t2) {
    ...
    if (e2.object.$parent == null) {
      const t3 = (_c = (_b = e2.parent.parent) == null ? void 0 : _b.object) == null ? void 0 : _c.$parent;
      if (t3 != null)
        e2.object.$parent = t3;
    }
    e2.property = n;
      ...
      if (this.template instanceof HTMLElement)
        if (typeof this.attribute === "boolean") {
          ...
          if (t2.length !== h.length)
            ...
            for (let r2 = 0, l2 = t2.length; r2 < l2; r2++) {
              let l3, o2;
              if (n == null) {
                if (t2[r2] !== h[r2]) {
                  e2 = true;
                ...
                l3 = s.resolveValue(t2[r2] ?? {}, n);
                o2 = s.resolveValue(h[r2] ?? {}, n);
                if (l3 !== o2)
                  e2 = true;
                if (t2[r2] !== h[r2])
                  i2 = true;
          ...
          if (o === r.length && r.length === l.length)
            return this.updateViews(t2, r);
          const u = this.views.splice(0, o);
          let a = u[u.length - 1];
          for (let t3 = o, e3 = r.length, i3 = o; t3 < e3; t3++, i3++) {
            const e4 = r[t3];
            ...
            const l2 = i4 == null ? void 0 : i4.model.$model[this.attribute];
            const o2 = n == null ? l2 : s.resolveValue(l2 ?? {}, n);
            if (h2 === o2) {
              u.push(i4);
              i4.move(a ?? this.element);
              a = i4;
              continue;
            }
            if (!r.slice(t3).map((t4) => n == null ? t4 : s.resolveValue(t4 ?? {}, n)).includes(o2)) {
              i4.destroy();
              t3--;
              a = i4;
              continue;
...
var _UI = class {
  ...
  static parse(t2, e2, i2, n) {
    ...
    if (t2.nodeType === 3) {
      ...
      while (r != null) {
        ...
        const a = o.match(this.regexConditionalValue);
        let c;
        let f = true;
        if (o.startsWith("(")) {
          const t3 = `_pui${this.bindingCounter++}`;
          Object.defineProperty(e2, t3, {
            get: new Function(`return ${o}`)
          });
          o = t3;
        } else if (a) {
          o = a[3];
      ...
      h.push(...Object.keys(t2.attributes ?? []).reverse().map((h2) => {
        ...
        if (t2 instanceof Comment)
          return [];
        const l = t2.attributes[h2];
        if (l.name.startsWith("pui.")) {
          const s2 = l.value.match(this.regexAttribute);
          let [h3, r3, o2, u2, a2] = s2;
          let c;
          let f;
          let p = false;
          if (o2 !== "@") {
            ...
            if (s3 != null) {
              ...
              if (u2 === ">") {
                const { target: i3, property: s4 } = this.resolveProperty(e2, a2);
                i3[s4] = t2;
                return [];
              } else {
                ({ element: t2, template: f } = this.replaceWithComment(t2, i2, n, l.name));
                r3 = o2 === "=";
                o2 = true;
                if (u2 === "|")
                  p = true;
              ...
              if (!t2.tagName.includes("-")) {
                ...
                if (e3.nodeType !== 8)
                  ({ element: t2, template: f } = this.replaceWithComment(t2, i2, n, l.name, f));
                else
                  t2 = e3;
              }
              f = r3;
        ...
        const o = [l.value];
        let u = 0;
        let a = o[u].match(this.regexValue);
        while (a != null) {
          ...
          r2.push(this.bind({
            ...
            toUI(e3, i3, n3, h4, r3) {
              if (this.oneTime) {
                const t3 = o.indexOf(n3);
                if (t3 > -1) {
                  o[t3] = s.resolveValue(h4, n3);
                  o[t3 - 1] += o[t3] + o[t3 + 1];
                  o.splice(t3, 2);
                }
              }
              const u2 = o.map((t3, i4) => {
                if (i4 % 2 === 0)
                  return t3;
                const l2 = t3.match(s.regexSplitConditionalValue);
                if (l2) {
                  const i5 = t3 === `${n3}${r3}` ? e3 : s.resolveValue(h4, l2[1]);
                  const o2 = l2[2] === "=";
                  return !!i5 === o2 ? l2[3].slice(1, -1) : "";
                }
                return t3 === n3 ? e3 : s.resolveValue(h4, t3);
              }).join("");
              t2.setAttribute(l.name, u2);
              return u2;
            },
            parent: i2,
            value: m
  ...
  static replaceWithComment(t2, e2, i2, s2, n) {
    const h = document.createComment(s2);
    this.parentNode(t2, i2).insertBefore(h, t2);
    this.parentNode(t2, i2).removeChild(t2);
    t2.removeAttribute(s2);
    n = n ?? t2;
    t2 = h;
    if (t2.parentNode instanceof DocumentFragment)
      e2.element = t2;
    return {
      element: t2,
      template: n
  ...
  static parseAttribute(t2, e2, i2, s2, n, h) {
    ...
    if (u !== "@") {
      ...
      if (h2 != null) {
        ...
        o = t2.nodeName.toLowerCase() === "option" ? "selected" : "checked";
        a = (t3) => t3 ? f : void 0;
        u = (t3) => t3 === f;
      } else if (o === "")
        if (a === ">") {
          const { target: i3, property: s3 } = this.resolveProperty(e2, c);
          i3[s3] = t2;
          return [];
        } else {
          ({ element: t2, template: p } = this.replaceWithComment(t2, i2, s2, n));
          o = u === "=";
          u = true;
          if (a === "|")
            m = true;
        ...
        if (e3.nodeType !== 8)
          ({ element: t2, template: p } = this.replaceWithComment(t2, i2, s2, n, p));
        else
          t2 = e3;
        p = o;
        m = true;
